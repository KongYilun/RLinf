...
from condition_model_sto import SiglipConditionModel
...
def main() -> None:
    ...
    print(f"num_classes: {num_classes}, residual_dim: {residual_dim}")
    # --- 3. 初始化条件编码模型 ---
    ...
    for epoch in range(num_epochs):
        ...
        for batch in dataloader:
            ...
            reg_mse_per = (residual_pred - residual_targets).pow(2).mean(dim=-1)
            sum_match = match_mask.float().sum()
            reg_loss = torch.where(
                sum_match > 0,
                (reg_mse_per * match_mask.float()).sum() / sum_match * 4000.0,
    ...
    # 可选：保存训练好的权重
    out_path = ROOT_DIR / "step3_encoder_training" / "siglip_condition_model_sto.pt"
    torch.save(
        {
