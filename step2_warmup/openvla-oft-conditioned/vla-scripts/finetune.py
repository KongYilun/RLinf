...
def get_run_id(cfg) -> str:
    ...
    if cfg.run_id_override is not None:
        ...
        if cfg.run_id_note is not None:
            ...
            run_id = f"{cfg.run_id_note}" # change 3 final
    return run_id
...
def run_forward_pass(
    ...
    # drop_mask = torch.rand(batch["input_ids"].size(0), device=device_id) < 0.25
    with torch.autocast("cuda", dtype=torch.bfloat16):
        output: CausalLMOutputWithPast = vla(
            ...
            use_film=use_film,
            z=batch["z"].to(device_id) if use_z else None,  # 只在 use_z=True 且 batch 有 z 时传
            # z_drop_mask = drop_mask,
        )
    # === 为 metrics 构造带 z 的 labels 视图 ===
    labels_for_metrics = batch["labels"].to(device_id)
    B, T = labels_for_metrics.shape
    ...
    if use_z:
        ...
        # new_labels_for_metrics = torch.empty(
        #     (B, T + 1),
        #     dtype=labels_for_metrics.dtype,
        #     device=labels_for_metrics.device,
        # )
        ...
        # for i in range(B):
        #     if not drop_mask[i]:
        #         new_labels_for_metrics[i]=torch.cat([pad_z_col[i], labels_for_metrics[i]], dim=0)
        #     else:
        #         new_labels_for_metrics[i]=torch.cat([labels_for_metrics[i], pad_z_col[i]], dim=0)
        # labels_for_metrics = new_labels_for_metrics

    # Get action masks needed for logging
    ground_truth_token_ids = labels_for_metrics[:, 1:].to(device_id)
    ...
    if not (use_l1_regression or use_diffusion):
        ...
        #     current_predicted_token_ids = 32064-65- predicted_token_ids[current_action_mask]
        #     current_ground_truth_token_ids = 32064-65- ground_truth_token_ids[current_action_mask]
        #     next_predicted_token_ids = 32064-65- predicted_token_ids[next_actions_mask]
        #     next_ground_truth_token_ids = 32064-65- ground_truth_token_ids[next_actions_mask]
        #     print(f"current predicted_token_ids \n { current_predicted_token_ids[0:14].tolist()}")
        #     print(f"current ground_truth_token_ids \n {current_ground_truth_token_ids[0:14].tolist()}")
            
        #     print(f"next predicted_token_ids \n {next_predicted_token_ids[0:14*10].tolist()}")
        #     print(f"next ground_truth_token_ids \n {next_ground_truth_token_ids[0:14*10].tolist()}")
...
def finetune(cfg: FinetuneConfig) -> None:
    ...
    val_steps = [0,10,20,40,80,160,320,640,1280,2000,3000] + list(range(4000,100000,1000))  #first change 
    assert cfg.use_lora, "Only LoRA fine-tuning is supported. Please set --use_lora=True!"
    # assert not (cfg.use_l1_regression and cfg.use_diffusion), (
    #     "Cannot do both L1 regression and diffusion. Please pick one of them!"
    # )
    if not (cfg.use_l1_regression and cfg.use_diffusion):
        action_head = None
    ...
    if distributed_state.is_main_process:
        wandb.init(entity=cfg.wandb_entity, project=cfg.wandb_project, name=f"ft+{run_id}",) #mode="offline"
    ...
    batch_transform = RLDSBatchTransform(
        ...
        use_proprio=cfg.use_proprio,
        # ==== 新增：z 文件路径 ====
        z_path="/data/users/kongyilun/code/RLinf/step1_embedding/libero_object_per_instruction_clusters_proj.pt",
    )
    ...
    with tqdm.tqdm(total=cfg.max_steps, leave=False) as progress:
        ...
        for batch_idx, batch in enumerate(dataloader):
            ...
            # if cfg.use_z:
            #     # 这里是“这个 batch 是否使用 z”
            #     use_z_for_this_batch = random.random() >= 0.25  # True: 用 z；False: 不用 z
            # else:
            #     use_z_for_this_batch = False
            ...
            if cfg.use_val_set and log_step >= 0 and log_step in val_steps:   #second change final
                run_validation(
                    vla=vla,
                    action_head=action_head,
                    noisy_action_projector=noisy_action_projector if cfg.use_diffusion else None,
