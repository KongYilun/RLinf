...
class PrismaticForConditionalGeneration(PrismaticPreTrainedModel):
    ...
    def forward(
        ...
        # === Handle Multimodal Forward ===
            ...
            # Get visual features (conditioned on language, if use_film=True). Note that z does NOT
            # participate in any vision or language feature computation; it will be appended later.
            ...
            if noisy_actions is not None:
                ...
                # print("input_embeddings before action mask", input_embeddings[:,:,:2])
                input_embeddings = input_embeddings * ~all_actions_mask
                # print("input_embeddings after action mask", input_embeddings[:,:,:2])

            # Build multimodal embeddings & attention mask (text + vision [+ proprio/diffusion])
            ...
            if z is not None:
                ...
                # # 2) 为所有样本分配新的 (S+1) 序列
                # new_embeddings = multimodal_embeddings.new_zeros(B, S + 1, D)
                # if multimodal_attention_mask is not None:
                #     new_attn = multimodal_attention_mask.new_zeros(B, S + 1)
                # else:
                #     new_attn = None
                # if multimodal_labels is not None:
                #     new_labels = multimodal_labels.new_full((B, S + 1), IGNORE_INDEX)
                # else:
                #     new_labels = None

                # # 3) 逐样本处理：未 drop 的样本在中间插 z；drop 的样本不插 z，只在末尾补 pad
                # for b in range(B):
                #     ip = insert_positions[b]

                #     if not z_drop_mask[b]:
                #         # === 未 drop：在 insert_pos 处插入 z ===
                #         # embeddings: [0:ip) + z + [ip:S)
                #         new_embeddings[b, :ip] = multimodal_embeddings[b, :ip]
                #         new_embeddings[b, ip] = z_token[b, 0]
                #         new_embeddings[b, ip + 1 : S + 1] = multimodal_embeddings[b, ip:S]

                #         if new_attn is not None:
                #             new_attn[b, :ip] = multimodal_attention_mask[b, :ip]
                #             new_attn[b, ip] = 1  # z 这个位置参与 attention
                #             new_attn[b, ip + 1 : S + 1] = multimodal_attention_mask[b, ip:S]

                #         if new_labels is not None:
                #             # z 的 label 永远是 IGNORE_INDEX
                #             new_labels[b, :ip] = multimodal_labels[b, :ip]
                #             new_labels[b, ip] = IGNORE_INDEX
                #             new_labels[b, ip + 1 : S + 1] = multimodal_labels[b, ip:S]
                #     else:
                #         # === 被 drop：不在中间插 z，只在最后补一个 pad token ===
                #         new_embeddings[b, :S] = multimodal_embeddings[b]
                #         # new_embeddings[b, S] 已经是 0，作为 pad embedding

                #         if new_attn is not None:
                #             new_attn[b, :S] = multimodal_attention_mask[b]
                #             new_attn[b, S] = 0  # 末尾 pad 不参与 attention

                #         if new_labels is not None:
                #             new_labels[b, :S] = multimodal_labels[b]
                #             new_labels[b, S] = IGNORE_INDEX  # 末尾 pad 的 label 也是 IGNORE_INDEX

                # multimodal_embeddings = new_embeddings
                # multimodal_attention_mask = new_attn
                # multimodal_labels = new_labels
...
class OpenVLAForActionPrediction(PrismaticForConditionalGeneration):
    ...
    def predict_action(
        ...
        **kwargs: str,
        ...
        # Extract language embeddings (non-action tokens only)
        language_embeddings = input_embeddings[~all_actions_mask].reshape(
            input_embeddings.shape[0], -1, input_embeddings.shape[2]
        )  # (B, lang_seq_len, D)

        # If provided, append z as an extra language token for FiLM conditioning
        if z is not None:
            # Accept z of shape (B, D) or (B, 1, D)
            if z.dim() == 2:
                z_token = z.unsqueeze(1)  # (B, 1, D)
            elif z.dim() == 3 and z.shape[1] == 1:
                z_token = z  # (B, 1, D)
            ...
            language_embeddings = torch.cat([language_embeddings, z_token], dim=1)

        # Process vision features (conditioned on language + optional z when use_film=True)
        projected_patch_embeddings = self._process_vision_features(pixel_values, language_embeddings, use_film)
