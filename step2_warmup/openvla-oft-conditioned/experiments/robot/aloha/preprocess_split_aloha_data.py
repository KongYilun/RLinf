...
def load_hdf5(demo_path):
    """Loads single episode with JPEG decoding."""
    ...
    with h5py.File(demo_path, "r") as root:
        ...
        for cam_name in root["/observations/images/"].keys():
            # 获取字节流数据 [num_frames,]
            byte_streams = root[f"/observations/images/{cam_name}"][()]  
            # 逐帧解码JPEG
            decoded_images = []
            for i in range(len(byte_streams)):
                np_arr = np.frombuffer(byte_streams[i], np.uint8)
                img = cv2.imdecode(np_arr, cv2.IMREAD_COLOR)
                img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)  # BGR转RGB
                decoded_images.append(img)
            image_dict[cam_name] = np.stack(decoded_images)
    print(f"Loading episode complete: {demo_path}")

    return qpos, action, image_dict, is_sim
...
def load_and_preprocess_all_episodes(demo_paths, out_dataset_dir):
    ...
    for demo in tqdm(demo_paths):
        qpos, action, image_dict, is_sim = load_hdf5(demo)
        # Save non-image info
